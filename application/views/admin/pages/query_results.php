            ...
            <div class="container">
                ...
                <div class="row no-gutters overflow-hidden">
                    ...
                    <div class="col-lg-2 pt-0">
                        <h3 class="page-title mb-2 mt-4"><?php echo config('saved_queries'); ?></h3>
                        <div class="pr-5">
                            <div class="list-group list-group-transparent mb-0">
                                <?php
                                    if (config('saved_queries') != NULL && $saved_query_list->rowCount() > 0) {
                                        for ($i = 0; $i < $saved_query_list->rowCount(); $i++) {
                                            $row = $saved_query_list->fetch();

                                            if ($id == $row->id) {
                                                echo '<a href="'.site_url("admin/database/saved_query/".$row->id).'" class="list-group-item list-group-item-action d-flex align-items-center active">
                                                    <span class="icon mr-3"><i class="'.$row->icon.'"></i></span>'.ucfirst($row->name).'
                                                    </a>';
                                            }
                                            else {
                                                echo '<a href="'.site_url("admin/database/saved_query/".$row->id).'" class="list-group-item list-group-item-action d-flex align-items-center">
                                                    <span class="icon mr-3"><i class="'.$row->icon.'"></i></span>'.ucfirst($row->name).'
                                                    </a>';
                                            }
                                        }
                                    ...
                                    if (!empty($table)) {
                                        echo '<a href="'.site_url('admin/database/insert_query/'.$table).'" class="list-group-item list-group-item-action d-flex align-items-center mt-3">
                                            <span class="icon mr-3"><i class="fa fa-plus-square-o"></i></span>Insert into '.ucfirst($table).'
                                            </a>';
                                    }

                                    if (!empty($ref_table)) {
                                        echo '<a href="'.site_url('admin/database/insert_query/'.$ref_table).'" class="list-group-item list-group-item-action d-flex align-items-center">
                                ...
                                <a href="<?php echo site_url('admin/database/edit_saved_query/queries/id/'.$id); ?>" class="list-group-item list-group-item-action d-flex align-items-center">
                                        ...
                                        for ($i = 0; $i < $query->rowCount(); $i++) {
                                            ...
                                            for ($j = 0; $j < $query->columnCount(); $j++) {
                                                ...
                                                if ($col['native_type'] == "BLOB") {
                                                    echo '<td><a href="'.site_url('admin/database/saved_query/'.$id.'/'.$value).'" class="text-dark table-link">'.get_summary($row[$col['name']], 2).'</a></td>';
                                                }
                                                else {
                                                    echo '<td><a href="'.site_url('admin/database/saved_query/'.$id.'/'.$value).'" class="text-dark table-link">'.$row[$col['name']].'</a></td>';
                                                }
                                            }
